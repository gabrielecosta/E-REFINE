...
class XGBTrainer:
    def __init__(self, data_dir, W, benign_label=0, attack_label=1, random_state=42):
        ...
        self.model = XGBClassifier(random_state=self.random_state, objective='binary:logistic', eval_metric='logloss')

    def train_eval_xgb(self, X_train, y_train):
    ...
    def save_model(self, w_index, folder_path):
        ...
        model_path = os.path.join(full_folder, f"xgb_model_{w_index}.pkl")
        with open(model_path, "wb") as f:
            pickle.dump(self.model, f)

    def load_model(self, w_index, folder_path):
        full_folder = f"{folder_path}/models/{w_index}"
        model_filename = f"xgb_model_{w_index}.pkl"
