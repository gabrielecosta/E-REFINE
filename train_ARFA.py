from ARF_adwin import ARFAModel
import pickle
import os 
...
class ARFATrainer:
    def __init__(self, data_dir, W, benign_label=0, attack_label=1, random_state=42):
        ...
        self.model = ARFAModel(random_state=random_state)

    def train_eval_arfa(self, X_train, y_train):
    ...
    def save_model(self, w_index, folder_path):
        ...
        model_path = os.path.join(full_folder, f"arfa_model_{w_index}.pkl")
        with open(model_path, "wb") as f:
            pickle.dump(self.model, f)

    def load_model(self, w_index, folder_path):
        full_folder = f"{folder_path}/models/{w_index}"
        model_filename = f"arfa_model_{w_index}.pkl"
